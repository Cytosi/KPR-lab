        ...
        for epoch in range(1, self.epochs + 1):
            for _, batch_data in enumerate(self.train_loader):
                ...
                print(
                    ...
                    f"{global_step}/{self.total_step} loss:{loss.item():.6f}"
                )
                global_step += 1
                ...
                if global_step % self.save_step == 0:
                    torch.save(
                        ...
                        os.path.join(self.output_dir, "pytorch_model_ner.bin"),
                    )
    ...
    def test(self):
        ...
        for batch_data in tqdm(self.test_loader):
            ...
            for i in range(batch_size):
                length = int(sum(attention_mask[i]))
                logit = logits[i][1 : length - 1]
                logit = [self.id2label[idx] for idx in logit]
                label = labels[i][1 : length - 1]
                label = [self.id2label[idx] for idx in label]
                preds.append(logit)
                trues.append(label)
...
def build_optimizer_and_scheduler(args, model, t_total):
    ...
    optimizer_grouped_parameters = [
        ...
        {
            "params": [
                p for n, p in other_param_optimizer if not any(nd in n for nd in no_decay)
            ],
            "weight_decay": args.weight_decay,
...
def main(data_name):
    ...
    if getattr(args, "dev_limit", None):
